...
## Author  : Izi
## Project :
## Created : sáb 11 fev 2023 16:13:46 -03
## License : MIT
## Updated :
##-------------------------------------------
...
stat_boxplot2 <- function(mapping = NULL, data = NULL,
                         geom = "boxplot2", position = "dodge2",
                         ...,
                         qs = NULL,
                         coef = 1e35,
                         na.rm = FALSE,
...
StatBoxplot2 <- ggproto("StatBoxplot2", Stat,
  ...
  setup_params = function(self, data, params) {
    ...
    if (!ggplot2:::is_mapped_discrete(data$x) && is.double(data$x) && !ggplot2:::has_groups(data) && any(data$x != data$x[1L])) {
      cli::cli_warn(c(
  ...
  compute_group = function(data, scales, width = NULL, na.rm = FALSE, qs = NULL, coef = 1e35, flipped_aes = FALSE) {
    ...
    if (length(ggplot2:::unique0(data$x)) > 1)

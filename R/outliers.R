...
outliers <- function(
  ...
  .class_ <- .extreme_ <- .outlier_ <- .obs. <- value_y <- NULL
  ...
  if (is.vector(data)) {
    ...
    value_name <- if (!is.call(value_name)) {
      deparse(value_name)
    } else {
      eval(value_name)
    }
  ...
  switch(language,
    "pt" = {
      ...
      if (is.null(name.classes)) {
        name.classes <- c("Regular", "Discrepante", "Extremo")
      }
    ...
    "en" = {
      ...
      if (is.null(name.classes)) {
        name.classes <- c("Regular", "Outlier", "Extreme")
      }
      names(color_class) <- c(name.classes, "NA")
      names(fill_class) <- c(name.classes, "NA")
      decimal.mark <- "."
  ...
  if (na.rm) {
    .DT. <- .DT.[is.finite(value_y)]
  }
  #' if (.DT.[, anyNA(value_y)]) {
  #'   warning(
  #'     "Data with NA/NaN/Inf!\n Consider removing using 'na.rm = TRUE'",
  #'     call. = FALSE
  #'   )
  #' }
  .DT.[, .obs. := seq_len(.N)]
  ...
  switch(method,
    ...
    "adjbox" = {
      ...
      if (MC < 0) {
        # Intervalo: [ Q25 - 1.5 exp(-3 * MC) * IQR;
        # Q75 + 1.5 exp(4 * MC) * IQR ]
        ...
        # Intervalo: [ Q25 - 1.5 exp(-4 * MC) * IQR;
        # Q75 + 1.5 exp(3 * MC) * IQR ]
  ...
  if (language == "en") {
    names(UL) <- c("lim", "fence")
  }

  if (!addthres[names_at[1]]) {
    UE <- NULL
  }
  if (!addthres[names_at[2]]) {
    U <- NULL
  }
  if (!addthres[names_at[3]]) {
    mediana <- NULL
  }
  if (!addthres[names_at[4]]) {
    L <- NULL
  }
  if (!addthres[names_at[5]]) {
    LE <- NULL
  }
  ...
  switch(remove,
    ...
    "none" = {
      .RM. <- NULL
      breaks_right <- sort(c(LE, L, mediana, U, UE))
      hline_up_e <- list(geom_hline(
        yintercept = c(UE),
        colour = "#BF3EFF",
        linetype = 5
      ))
      hline_low_e <- list(geom_hline(
        yintercept = c(LE),
        colour = "#BF3EFF",
        linetype = 5
      ))
  ...
  hline_up <- list(geom_hline(
    yintercept = c(U),
    colour = "#458B74",
    linetype = 5
  ))
  hline_low <- list(geom_hline(
    yintercept = c(L),
    colour = "#458B74",
    linetype = 5
  ))
  ...
  if (length(col_names) > 0) {
    nc <- suppressWarnings(max(
      as.integer(unlist(strsplit(col_names, "\\."))),
      na.rm = TRUE
    ))
    if (is.infinite(nc)) {
      nc <- 0
  ...
  p <- ggplot(
    .DT.,
    aes(x = .obs., y = eval(as.name(value_name)), fill = eval(as.name(cls)))
  ) +
    geom_point(size = 2, shape = 21, color = "black", na.rm = TRUE) +
    geom_text(
      data = .DT.[eval(as.name(ext)) == TRUE],
      aes(label = .obs.),
      size = 2.7,
      colour = "black",
      check_overlap = TRUE,
    ...
    theme(
      ...
      #' plot.margin = unit(x = c(t = 0, r = 0, b = 0, l = 0),
      #' units = "points"),
    ...
    labs(
      ...
      subtitle = paste0(
        ...
        n_extremo,
        lang[4],
        n_extremo + n_discrepante,
        " (",
        format_number(n_perc, 0, suffix = "%"),
        ")",
        "\nNA = ",
        n_na,
        lang[5],
  ...
  if (language == "en") {
    names(out) <- c("plot", "data", "removed", "ul", "summary")
  }
